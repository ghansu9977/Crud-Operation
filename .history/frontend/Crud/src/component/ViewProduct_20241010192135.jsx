    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                // const response = await axios.get('http://localhost:3000/view');
                // dispatch(setProducts(response.data));
                const filteredItems = items.filter(item => item.pname.toLowerCase().includes(searchTerm.toLowerCase()));
                if (filteredItems.length === 0 && currentPage > 1) {
                    setCurrentPage(currentPage - 1); // Go to the previous page if no items left
                }
            } catch (error) {
